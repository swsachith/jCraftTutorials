...
public class RemoteCommandExecutor {
    public static void main(String[] arg) {
        ...
        try {
            ...
            command = "ls";
            executeCommand(session, c, out, cmds,command);
            session.disconnect();
        } catch (Exception e) {
    ...
    private static void executeCommand(Session session, ChannelSftp c, PrintStream out, Vector cmds,String command) throws Exception {
        String str;
...
//                String cmd=(String)cmds.elementAt(0);
            ...
            if (command.equals("cd") || command.equals("lcd")) {
                if (cmds.size() < 2) continue;
                String path = (String) cmds.elementAt(1);
                try {
                    if (command.equals("cd")) c.cd(path);
                    else c.lcd(path);
            ...
            if (command.equals("rm") || command.equals("rmdir") || command.equals("mkdir")) {
                if (cmds.size() < 2) continue;
                String path = (String) cmds.elementAt(1);
                try {
                    if (command.equals("rm")) c.rm(path);
                    else if (command.equals("rmdir")) c.rmdir(path);
                    else c.mkdir(path);
                } catch (SftpException e) {
                    System.out.println(e.toString());
                }
            ...
            if (command.equals("chgrp") || command.equals("chown") || command.equals("chmod")) {
            ...
            if (command.equals("pwd") || command.equals("lpwd")) {
                str = (command.equals("pwd") ? "Remote" : "Local");
                str += " working directory: ";
                if (command.equals("pwd")) str += c.pwd();
                else str += c.lpwd();
                out.println(str);
                continue;
            }
...
//		out.println(vv.elementAt(ii).toString());
            ...
            if (command.equals("lls") || command.equals("ldir")) {
                String path = ".";
                if (cmds.size() == 2) path = (String) cmds.elementAt(1);
                try {
            ...
            if (command.equals("get") ||
                    command.equals("get-resume") || command.equals("get-append") ||
                    command.equals("put") ||
                    command.equals("put-resume") || command.equals("put-append")
                    ) {
...
//	  String p2=p1;
                ...
                try {
                    ...
                    if (command.startsWith("get")) {
                        ...
                        if (command.equals("get-resume")) {
                            mode = ChannelSftp.RESUME;
                        } else if (command.equals("get-append")) {
                            mode = ChannelSftp.APPEND;
                        }
                        c.get(p1, p2, monitor, mode);
                    } else {
                        int mode = ChannelSftp.OVERWRITE;
                        if (command.equals("put-resume")) {
                            mode = ChannelSftp.RESUME;
                        } else if (command.equals("put-append")) {
                            mode = ChannelSftp.APPEND;
                        }
            ...
            if (command.equals("ln") || command.equals("symlink") ||
                    command.equals("rename") || command.equals("hardlink")) {
                if (cmds.size() != 3) continue;
                String p1 = (String) cmds.elementAt(1);
                String p2 = (String) cmds.elementAt(2);
                try {
                    if (command.equals("hardlink")) {
                        c.hardlink(p1, p2);
                    } else if (command.equals("rename")) c.rename(p1, p2);
            ...
            if (command.equals("stat") || command.equals("lstat")) {
                if (cmds.size() != 2) continue;
                String p1 = (String) cmds.elementAt(1);
                SftpATTRS attrs = null;
                try {
                    if (command.equals("stat")) attrs = c.stat(p1);
            ...
            out.println("unimplemented command: " + command);
        }
    }
